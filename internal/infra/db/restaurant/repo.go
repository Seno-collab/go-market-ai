...
func (rr *RestaurantRepo) Update(ctx context.Context, r *restaurant.Entity, id int32) error {
	...
	err = qtx.DeleteRestaurantHours(ctx, id)
	if err != nil {
		return err
	}
	for _, h := range r.Hours {
...
func (rr *RestaurantRepo) Delete(ctx context.Context, id int32) error {
	tx, err := rr.pool.Begin(ctx)
	if err != nil {
		return err
	}
	defer tx.Rollback(ctx)
	qtx := rr.q.WithTx(tx)
	if err := qtx.DeleteRestaurant(ctx, id); err != nil {
		return err
	}
	if err := qtx.DeleteRestaurantHours(ctx, id); err != nil {
		return err
	}
	if err := tx.Commit(ctx); err != nil {
		return err
